...
export default function PhotoGalleryManager({ gallery }: PhotoGalleryManagerProps) {
  ...
  const onAddPhoto = async (values: PhotoFormValues) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || "Произошла ошибка при добавлении фотографии");
      }
  ...
  const onAddBatchPhotos = async (values: BatchPhotoFormValues) => {
    ...
    try {
      ...
      const photosToAdd = urls.map(url => ({ url }));
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || "Произошла ошибка при добавлении фотографий");
      }

      const result = await response.json();

      if (result.errors && result.errors.length > 0) {
        toast.warning(`Добавлено ${result.created.length} из ${photosToAdd.length} фотографий. Некоторые URL некорректны.`);
      } else {
  ...
  const onEditPhoto = async (values: PhotoFormValues) => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || "Произошла ошибка при обновлении фотографии");
      }
